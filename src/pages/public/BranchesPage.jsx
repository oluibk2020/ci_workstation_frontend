...
export default function BranchesPage() {
  const { branches, getWorkstationsForBranch, getSeatsForWorkstation } =
    useCatalog();
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 sm:py-20">
      ...
      <p className="mt-3 max-w-2xl text-slate-500">
        Pricing varies by branch and by desk type — a Standing Desk in one
        branch can cost differently from another, set by our admin team based on
        that branch's running costs.
      </p>

      <div className="mt-10 grid gap-6 sm:grid-cols-2">
        {branches.map((branch) => {
          const workstations = getWorkstationsForBranch(branch.id);
          const seatCount = workstations.reduce(
            (sum, wk) => sum + getSeatsForWorkstation(wk.id).length,
            0,
          );
            ...
            >
              ...
              <div className="p-6">
                <div className="flex items-center gap-2 text-[var(--color-accent)]">
                  <MapPin size={18} />
                  <p className="font-mono-tight text-xs font-semibold uppercase tracking-wide">
                    {branch.name}
                  </p>
                </div>
